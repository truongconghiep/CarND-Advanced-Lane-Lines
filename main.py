...
thresholded_img = color_Gradient_Threshold(img, plot_allowed = False)
# Perspective transformation
M, Minv = Perspective_Transformation(undistorted_img, plot_allowed = False)
# Pipeline for images
img_p = Pipeline_image(img, M, mtx, dist, Minv, plot_allowed = False)
...
def process_image(image):
    global frame_number, left_fit, right_fit, last_left_curverad, last_right_curverad, last_offset, Is_1st_Frame_b
    left_fit, right_fit, img, Detected, new_left_curverad, new_right_curverad, new_offset = Pipeline_video(image, M, Minv,mtx, dist, 
                                                       Is_1st_Frame = Is_1st_Frame_b,
                                                       left_fit = left_fit,
                                                       right_fit = right_fit,
...
file_name =  input_harder_challenge_video.replace('./test_videos/', '')
